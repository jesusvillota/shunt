...
| `GET` | `/admin/pool` | JSON: per-`claude_oauth`/`chatgpt_oauth`/`antigravity_oauth` managed-pool state; account objects may include an optional `plan` string (`antigravity_oauth` accounts additionally carry a `quota_buckets` array of per-model quota buckets from the `retrieveUserQuota` poll) |
