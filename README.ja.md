...
モデル id をひとつ、自分で判断させることもできます。[`[models.router]`](https://shunt.sh/ja/guides/stage-router/) エントリは `type` キーでルーティングアルゴリズムを指定します。`stage_router` は強力なティアと効率的なティアを指定し、会話の直近の **tool-result メタデータ**（プロンプトのテキストではなく `tool_use.name` と `tool_result.is_error`）からターンごとにどちらかを選びます。`auto` は同じルーターをアップストリームのプリセットで動かし、`random` は重み付きのターゲットにトラフィックを振り分けつつ 1 つのセッションを同じ枝に留め、`noop` は空のメッセージを返すのでスモークテストに使えます。`prefill_router` は直近のユーザーターンを読む学習済み分類器で、既定で無効な `prefill-router` カーゴフィーチャーを有効にしてビルドしたバイナリでのみ使えます。判断を **LLM ジャッジ** に委ねるタイプも 2 つあります。[`llm_classifier`](https://shunt.sh/ja/reference/configuration/#type--llm_classifier) はジャッジの判定どおりにルーティングし、`mode = "capability"` はジャッジが出した解決確率で強力なターゲットと効率的なターゲットのどちらかを選び、`mode = "custom"` は自分で書いたプロンプトと JSON スキーマが指し示すモデルグループを選びます。[`composite`](https://shunt.sh/ja/reference/configuration/#type--composite) は `stage_router` のシグナル採点はそのままに、シグナルで決まらなかったときに落ちるティアだけをジャッジに決めさせます。`stage_router` にはジャッジモデルを指定する [`[models.router.classifier]`](https://shunt.sh/ja/reference/configuration/#modelsrouterclassifierオプション) テーブルを任意で追加できます。このジャッジはシグナルで決まらなかったターンでのみ呼ばれ、クライアントに提供されることはありません。ジャッジを呼ぶタイミングは `classify_trigger` キーが決め（`every_request`、`user_turn`、`new_session`）、`judge_*`/`gated_*`/`max_judge_calls` の 6 つのキーが内部呼び出しごとに上限を課します。ターゲットはいずれも通常の公開モデル id なので、それぞれのフェイルオーバーチェーン、プール、アダプターをそのまま引き継ぎます（[Switchyard 統合](https://shunt.sh/ja/guides/switchyard/)）。どのエントリも [`[models.subagents]`](https://shunt.sh/ja/reference/configuration/#modelssubagentsオプション) オーバーレイを併せて持てます。これは委譲された作業 — `Task` サブエージェント、フックエージェント、ワークフローサブエージェント — を別のターゲットへ送るもので、エージェントタイプごとに指定することもでき（`by_type = { Explore = "claude-haiku-4-5" }`）、親セッション自身の宛先はそのままです。`main`、コンパクション、補助のターンがこれを使うことはありません。このオーバーレイにはもう 1 つの形、[`type = "llm_classifier"` と `mode = "custom"`](https://shunt.sh/ja/reference/configuration/#subagents-type--llm_classifier) があります。ジャッジが委譲されたタスクを読み、(セッション, エージェント) の組ごとに一度だけ子のターゲットを選びます。親が判定されることはありません。ルーターも subagents オーバーレイも設定しなければ挙動は変わりません。
