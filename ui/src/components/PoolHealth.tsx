...
export function PoolHealth({ pool, onMutated }: PoolHealthProps): ReactElement {
  ...
  async function togglePause(provider: string, name: string, paused: boolean): Promise<void> {
    try {
      const result = await patchPoolAccount(csrf, provider, name, paused);
      if (!result.ok) {
                  ...
                  {canWrite ? (
                    <td className="row-actions">
                      <button
                        ...
                        onClick={() => void togglePause(provider, account.name, !account.paused)}
                      >
