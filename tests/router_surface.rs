...
/// `/admin/api/pool/{provider}/accounts/{name}` register it (pool pause and
...
const ADMIN_PATHS: [(&str, &str); 24] = [
    ...
    ("/admin/api/pool/{provider}/accounts/{name}", "PATCH"),
...
fn the_source_scan_finds_every_literal_registration() {
    ...
    // so only the new `/admin/api/pool/{provider}/accounts/{name}` literal
