...
| `usage_refresh_seconds` | disabled (`0`/absent) | Poll interval, in seconds, for Claude `GET /api/oauth/usage`, Codex `GET /wham/usage`, and Antigravity `POST :retrieveUserQuota`; a positive value below 60 is clamped up to a 60-second floor |
...
For each window `X`, the effective soft threshold resolves as: account `threshold_X` → account `threshold` → `default_threshold_X` → `default_threshold` → `hard_threshold`, and is capped at `hard_threshold`. All thresholds are utilization fractions in `[0.0, 1.0]`; out-of-range values fail startup. The threshold and burn-rate knobs govern both pool families: the Anthropic pool from its `anthropic-ratelimit-unified-*` headers, and the Codex/ChatGPT pool from its `x-codex-*` 5-hour/weekly windows (Codex has no Fable-scoped `7d_oi` window, so `default_threshold_fable` is inert there). `usage_refresh_seconds` polls all three families: Anthropic accounts use the official API, imported Codex/ChatGPT-backend accounts use the private, unofficial `wham/usage` endpoint ([details](/guides/codex-multi-account/#usage-poller)), and imported `antigravity_oauth` accounts use the Code Assist `retrieveUserQuota` RPC, which reports per-model quota buckets (each model with its own remaining fraction and reset time) rather than account-wide windows — the account's only quota signal, surfaced display-only on the dashboard without changing pool selection.
