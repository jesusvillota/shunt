...
| `GET` | `/routes` | shunt-native route discovery — returns the configured `[[routes]]` table verbatim (model → provider/upstream_model/effort mapping, including claude-prefixed discovery aliases); distinct from `/v1/models`, which serves the narrower Anthropic-protocol discovery response (`id`, `display_name`, and upstream model metadata). A `routers` array is added when at least one `[[models]]` entry configures a `[models.router]` table, naming each router's model id, its `algorithm` (the configured `type`), and every `targets` id it can route to; a router that configures a judge also reports `judges`, the ids it consults but never serves (omitted when it has none); a [stage router](/guides/stage-router/) or `auto` entry additionally reports its two tier targets and the tier its picker falls back to when no signal decides |
...
| `GET` | `/admin/api/session` | The three per-session values the admin SPA needs before it can render: the session's `csrf` token, `expiry_buffer_ms` (`claude::auth::EXPIRY_BUFFER` in milliseconds, the boundary at which a setup token stops being usable), and `access` — `read` or `write`, the tier the session authenticates with, which is what the dashboard renders its write affordances from. A header-credential caller is CSRF-exempt and receives an empty `csrf`. Returning the token over a `GET` is safe because no CORS layer exists on this surface, so a cross-origin page can send the request but cannot read the reply |
...
| `GET` | `/admin/api/accounts/antigravity` | Antigravity account-store metadata: name, expiry, and email (when Google returned one); never token material |
...
| `POST` | `/admin/api/accounts/antigravity` | Start Antigravity OAuth with `{name}`; returns `{authorize_url}` |
| `POST` | `/admin/api/accounts/antigravity/{name}/complete` | Complete Antigravity provisioning with `{code}` containing the full localhost redirect URL or `<code>#<state>`; stores the account and reports whether it is live |
| `POST` | `/admin/api/accounts/antigravity/{name}/refresh` | Exercise an Antigravity account's refresh grant on demand and report whether the login is still alive, the same probe shape as the Claude refresh route above |
| `DELETE` | `/admin/api/accounts/antigravity/{name}` | Remove the named Antigravity account's store file |
...
else moved, so that the dashboard can claim `/admin/*` as browsable deep links: a
path cannot be both a JSON endpoint and a client-side route, which is why no
compatibility alias was kept.

Rewrite scripted callers once, mechanically — the path gains one `/api` segment
after `/admin`:
...
A Claude Code client can read this without reaching for `curl`: the [`shunt` mod](https://github.com/pleaseai/shunt/tree/main/plugins/shunt) in this repository's plugin marketplace answers `/shunt:usage` from this endpoint, printing each window's headroom and reset. It needs function hooks (`CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`), which are early access.
