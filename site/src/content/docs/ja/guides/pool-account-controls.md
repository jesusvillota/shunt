...
| :-- | :-- | :-- |
| 設定方法 | `shunt.toml`、リロードが必要 | 管理ダッシュボードまたは `PATCH /admin/api/pool/{provider}/accounts/{name}` |
| 再起動後も維持 | はい | いいえ |
...
再開するには `"paused": false` を設定します。エンドポイントの完全なリファレンスは [`PATCH /admin/api/pool/{provider}/accounts/{name}`](/ja/reference/endpoints/) を参照してください。
