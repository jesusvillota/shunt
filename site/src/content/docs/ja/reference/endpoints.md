...
| `PATCH` | `/admin/api/pool` | write 権限限定。ボディ `{"sort_by_reset": true\|false}` でプール全体のリセット優先ソートをランタイムに切り替える(`[server.pool]` は provider ごとではなくプロセス全体であるため)。利用可能なアカウントはバーンレートのヘッドルームではなくクォータのリセットが最も早い順に並ぶ。メモリ上のみで、解除するか再起動すると設定ファイル自身の値に戻る |
