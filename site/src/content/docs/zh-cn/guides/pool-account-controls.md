...
| :-- | :-- | :-- |
| 设置方式 | `shunt.toml`,需要重载 | 管理仪表盘或 `PATCH /admin/api/pool/{provider}/accounts/{name}` |
| 重启后是否保留 | 是 | 否 |
...
设置 `"paused": false` 即可恢复。完整端点参考见 [`PATCH /admin/api/pool/{provider}/accounts/{name}`](/zh-cn/reference/endpoints/)。
