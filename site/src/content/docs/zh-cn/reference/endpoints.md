...
| `PATCH` | `/admin/api/pool` | 仅限 write 级别。请求体 `{"sort_by_reset": true\|false}` 在运行时为整个账户池切换重置优先排序(`[server.pool]` 是进程级的,不是按 provider 划分):可用账户随后按配额最快重置排序,而不是按燃烧速率余量排序。只存在于内存中;被清除或重启后恢复为配置文件自身的值 |
