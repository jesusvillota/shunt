...
| :-- | :-- | :-- |
| 설정 방법 | `shunt.toml`, 리로드 필요 | 관리자 대시보드 또는 `PATCH /admin/api/pool/{provider}/accounts/{name}` |
| 재시작 후 유지 | 예 | 아니오 |
...
재개하려면 `"paused": false`로 설정하세요. 전체 엔드포인트 참조는 [`PATCH /admin/api/pool/{provider}/accounts/{name}`](/ko/reference/endpoints/)를 참고하세요.
