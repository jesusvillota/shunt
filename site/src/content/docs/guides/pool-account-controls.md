...
| :-- | :-- | :-- |
| Set via | `shunt.toml`, reloaded | Admin dashboard or `PATCH /admin/api/pool/{provider}/accounts/{name}` |
...
```bash
curl -X PATCH "$SHUNT_URL/admin/api/pool/anthropic/accounts/my-account" \
  -H "x-shunt-admin-token: $ADMIN_TOKEN" \
  -H "content-type: application/json" \
  -d '{"paused": true}'
```

Set `"paused": false` to resume. See [`PATCH /admin/api/pool/{provider}/accounts/{name}`](/reference/endpoints/) for the full endpoint reference.
